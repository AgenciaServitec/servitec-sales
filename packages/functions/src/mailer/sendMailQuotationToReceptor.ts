...
export const sendMailQuotationToReceptor = async ({quotation}:Props): Promise<void> => {
    assert(quotation, "Missing quotation");
    const client: Client | undefined = await fetchClientByHostname(quotation.hostname);
...
const fetchClientByHostname = async (hostname: string): Promise<Client | undefined> => {
